...
public class AdvancedScheduleScreen extends AbstractSimiContainerScreen<AdvancedScheduleMenu> {
    ...
    private ScrollInput hourInput;
    private ScrollInput minuteInput;
    ...
    protected void init() {
        ...
        timeLabel = new Label(x, y + 2, CommonComponents.EMPTY).withShadow();

        hourInput = new ScrollInput(x, y, 14, 12).withRange(0, 24)
                .titled(Component.literal("Hour"))
                .calling(h -> {
                    timeLabel.text = getDigitalDisplay(h, minuteInput.getState());
                })
                .setState(18);
        minuteInput = new ScrollInput(x + 16, y, 14, 12).withRange(0, 60)
                .titled(Component.literal("Minute"))
                .calling(m -> {
                    timeLabel.text = getDigitalDisplay(hourInput.getState(), m);
                })
                .setState(30);

        timeLabel.text = getDigitalDisplay(hourInput.getState(), minuteInput.getState());
    ...
    protected void startEditing(IScheduleInput field, Consumer<Boolean> onClose, boolean allowDeletion) {
        ...
        if (hourInput != null) {
            hourInput.visible = false;
            minuteInput.visible = false;
    ...
    protected void stopEditing() {
        ...
        if (hourInput != null) {
            hourInput.visible = true;
            minuteInput.visible = true;
    ...
    public int renderScheduleEntry(GuiGraphics graphics, ScheduleEntry entry, int yOffset, int mouseX, int mouseY,
        ...
        int inputWidth = renderInput(graphics, destination, 26, 5, false, 100);
        entry.instruction.renderSpecialIcon(graphics, 30, 5);

        String timeLabelText = "18:30";
        Component timeComponent = Component.literal(timeLabelText);
        int timeLabelWidth = font.width(timeComponent);
        int availableSpace = cardWidth - inputWidth;
        Font font1 = font;

        graphics.drawString(font, timeLabelText, cardWidth - timeLabelWidth-20, 4, 0xFFFFFFFF);
        graphics.drawString(font, timeLabelText, cardWidth - timeLabelWidth-20, 14, 0xFFFFFFFF);
    ...
    protected int renderInput(GuiGraphics graphics, Pair<ItemStack, Component> pair, int x, int y, boolean clean,
        ...
        if (minSize == 100 && y == 5) {
            ...
            if (text != null) {
                String truncated = font.plainSubstrByWidth(text.getString(), 60);
                pair = Pair.of(pair.getFirst(), Component.literal(truncated));
            }
        }
        ItemStack stack = pair.getFirst();
        Component text = pair.getSecond();
        boolean hasItem = !stack.isEmpty();
        int fieldSize = Math.min(getFieldSize(minSize, pair), 100);
        PoseStack matrixStack = graphics.pose();
        matrixStack.pushPose();
        ...
        if (text != null)
            graphics.drawString(font, font.substrByWidth(text, 100)
    ...
    private Component getDigitalDisplay(int hour, int minute) {
        return Component.literal((hour < 10 ? "0" : "") + hour + ":" + (minute < 10 ? "0" : "") + minute);
    }
