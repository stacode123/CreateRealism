    ...
    private void createBlockEdge(TrackGraph graph, BlockNode start, BlockNode end, List<TrackEdge> path, BlockGraph blockGraph, boolean reverse) {
        ...
        if (!reverse) {
            for (int i = 0; i < path.size(); i++) {
                ...
                trackEdge.getEdgeData().getPoints().forEach(point -> {
                    if (point instanceof GlobalStation station && !point.isPrimary(trackEdge.node1)) {
                        stations.add(new BlockStation(null, station, station.id, blockGraph, currentLength + station.getLocationOn(trackEdge)));
                    }
                });
            ...
            for (int i = path.size() - 1; i >= 0; i--) {
                ...
                trackEdge.getEdgeData().getPoints().forEach(point -> {
                    if (point instanceof GlobalStation station && !point.isPrimary(trackEdge.node1)) {
                        edgeStations.add(new BlockStation(null, station, station.id, blockGraph, currentLength + (trackEdge.getLength() - station.getLocationOn(trackEdge))));
                    }
                });
                // If multiple stations on same edge, they should be in reverse order for reverse traversal
                edgeStations.sort(Comparator.comparingDouble((BlockStation s) -> s.location));
    ...
    private void createBlockEdge(TrackGraph graph,BlockGraph blockGraph,BlockNode start, BlockNode end,List<TrackEdge> edges,SignalBoundary p1,SignalBoundary p2) {
        ...
        if (p1loc <= p2loc) {
            for (int i = 0; i <= p2loc; i++) {
                ...
                final double distanceAtStartOfEdge = currentDistance;
                if (i >= p1loc) {
                    List<BlockStation> edgeStations = new ArrayList<>();
                    edge.getEdgeData().getPoints().forEach(point -> {
                        if (point instanceof GlobalStation station && !point.isPrimary(edge.node1)) {
                            ...
                            edgeStations.add(new BlockStation(null, station, station.id, blockGraph, distanceAtStartOfEdge + loc - (currentIndex == p1loc ? p1.getLocationOn(edge) : 0)));
                        }
                    });
                    // Sort stations by location on edge (assuming p1loc to p2loc is forward)
                    edgeStations.sort(Comparator.comparingDouble(s -> s.location));
                    blockStations.addAll(edgeStations);
                }
                if (i == p1loc) {
                    currentDistance += (edge.getLength() - p1.getLocationOn(edge));
                } else if (i > p1loc && i < p2loc) {
            ...
            for (int i = p1loc; i >= p2loc; i--) {
                ...
                edge.getEdgeData().getPoints().forEach(point -> {
                    if (point instanceof GlobalStation station && !point.isPrimary(edge.node1)) {
                        ...
                        if (currentIndex == p1loc && loc >= p1.getLocationOn(edge)) return;
                        if (currentIndex == p2loc && loc <= p2.getLocationOn(edge)) return;
                        edgeStations.add(new BlockStation(null, station, station.id, blockGraph, station.getLocationOn(edge)));
                    }
                });
                ...
                if (i == p1loc) {
                    currentDistance += p1.getLocationOn(edge);
                } else if (i < p1loc && i > p2loc) {
                    currentDistance += edge.getLength();
                }
            }
            ...
            for (int i = 0; i <= ploc; i++) {
                ...
                edge.getEdgeData().getPoints().forEach(point -> {
                    if (point instanceof GlobalStation station && !point.isPrimary(edge.node1)) {
                        ...
                        double location = distanceAtStartOfEdge +loc;
                        edgeStations.add(new BlockStation(null, station, station.id, blockGraph, distanceAtStartOfEdge + loc));
                    }
                });
                edgeStations.sort(Comparator.comparingDouble(s -> s.location));
                blockStations.addAll(edgeStations);
