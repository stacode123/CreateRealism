    ...
    public static void visualize(BlockGraph graph, String filename) {
        ...
        for (BlockEdge edge : graph.edges) {
            ...
            if (edge.stations != null && !edge.stations.isEmpty()) {
                double midX = (x1 + x2) / 2.0 + offsetX;
                double midZ = (z1 + z2) / 2.0 + offsetZ;
                svg.append(String.format("  <rect x=\"%f\" y=\"%f\" width=\"1.5\" height=\"1.5\" fill=\"green\" stroke=\"black\" stroke-width=\"0.1\" transform=\"translate(-0.6, -0.6)\" />\n",
                        midX, midZ));
            }
        }
